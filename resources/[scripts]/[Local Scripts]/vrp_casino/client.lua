...
function vRPcasinoC.createCasinoText(x, y, z, name)
  while true do
  Citizen.Wait(0)
  --DrawMarker(29,x,y,z-1, 0, 0, 0, 0, 0, 0, 1.5001, 1.5001, 0.6001,255,0,0, 200, 0, 0, 0, 0)
  DrawMarker(29, x, y, z-0.5, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 1.0, 0, 200, 200, 165, 0, 0, 0, true)
  --DrawMarker(1, x, y, z, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 1.0, 0, 25, 165, 165, 0, 0, 0, true)
...
Citizen.CreateThread(function()
	while true do
		...
		if(rltBetOn ~= "")then
			text_drawTxt("다이아몬드 룰렛",1.0,1.0,0.5,0.75,0.5,255,255,255,255)
			text_drawTxt("["..rltNumber.."~w~]",1.0,1.0,0.5,0.8,0.5,255,255,255,255)
		end
	end
end)
...
local moneymachine_slot = {
	{ ['x'] = 1115.7431640625, ['y'] = 229.04878234863,['z'] = -49.840789794922 },
	{ ['x'] = 1112.6127929688, ['y'] = 233.83712768555,['z'] = -49.840763092041 },
	{ ['x'] = 1108.3706054688, ['y'] = 233.01667785645,['z'] = -49.840763092041 },
	{ ['x'] = 1104.9318847656, ['y'] = 228.09944152832,['z'] = -49.840763092041 },
			...
			if(GetDistanceBetweenCoords(v.x, v.y, v.z, pos.x, pos.y, pos.z, true) < 20.0)then
				...
				if(GetDistanceBetweenCoords(v.x, v.y, v.z, pos.x, pos.y, pos.z, true) < 1.0)then
					if(Menu.hidden == true)then
						text_drawTxt("SLOT PLAY ~g~[E]",4,1,0.5,0.8,0.7,255,255,255,255)
					end
					...
					if not Menu.hidden then
						...
						drawText(0.55, 0.42+0.5, 1.0,1.0,0.6,'~w~칩 ~g~'..tostring(money).."~w~ 개",255,0,0,255,7)
					end
					Menu.renderGUI()
				end
			end
