...
AddEventHandler("Fax:ServerPassword:ShowPasswordPrompt", function()
    --local password = KeyboardInput("접속 암호를 입력해주세요 (" .. attempts .. " 시도 " .. (attempts == 1 and "" or "s") .. " 번 남음)", "", 30)
    local password = KeyboardInput("접속 암호를 입력해주세요 (" .. attempts .. "번 시도 남음)", "", 30)
    attempts = attempts - 1
...
AddEventHandler('Fax:ServerPassword:PassedPassword', function()
    ...
    TriggerEvent('chatMessage', "^2암호가 일치합니다.")
end)
