--20190718 Vehicle Push ESX to VRP by Gryffin
      ...
      while true do 
          ...
          if Vehicle.Vehicle ~= nil then
                  ...
                  if IsVehicleSeatFree(Vehicle.Vehicle, -1) and GetVehicleEngineHealth(Vehicle.Vehicle) <= Config.DamageNeeded then
                     DrawText3D({x = Vehicle.Coords.x, y = Vehicle.Coords.y, z = Vehicle.Coords.z}, '차량을 밀고 가려면 [~g~SHIFT~w~] 와 [~g~E~w~] 를 눌러 조작하세요!', 0.6) 
                  end
       
  
              if IsControlPressed(0, Keys["LEFTSHIFT"]) and IsVehicleSeatFree(Vehicle.Vehicle, -1) and IsVehicleSeatFree(Vehicle.Vehicle, -1) and not IsEntityAttachedToEntity(ped, Vehicle.Vehicle) and IsControlJustPressed(0, Keys["E"])  and GetVehicleEngineHealth(Vehicle.Vehicle) <= Config.DamageNeeded then
