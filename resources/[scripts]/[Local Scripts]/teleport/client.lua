--Allright Reserved 2018 KR RP The Raven Life--
--Maker : NOEL--


local TeleportFromTo = {
	["Police HeliPad"] = {
		positionFrom = { ['x'] = 452.4290, ['y'] = -993.8731, ['z'] = 25.9989, nom = "헬기장"},
		positionTo = { ['x'] = 469.1937, ['y'] = -986.9882, ['z'] = 43.6916, nom = "차고"},
	},

	["Police HeliPad2"] = {
		positionFrom = { ['x'] = -1042.3129, ['y'] = -828.0255, ['z'] = 10.8846, nom = "헬기"},
		positionTo = { ['x'] = -1106.9100, ['y'] = -832.7482, ['z'] = 37.6753, nom = "차고"},
	},
	
	["Maze Bank"] = {
		positionFrom = { ['x'] = -70.799667358398, ['y'] = -800.20782470703, ['z'] = 44.227291107178, nom = "옥상"},
		positionTo = { ['x'] = -75.116439819336, ['y'] = -824.67926025391, ['z'] = 321.29211425781, nom = "1층"},
	},
	
	["Bunker"] = {
		positionFrom = { ['x'] = 781.46527099609, ['y'] = 1296.6982421875, ['z'] = 361.36181640625, nom = "벙커 입구"},
		positionTo = { ['x'] = 963.90124511719, ['y'] = -3245.9389648438, ['z'] = -93.67017364502, nom = "밖"},
	},
}
			...
			if(Vdist(pos.x, pos.y, pos.z, j.positionFrom.x, j.positionFrom.y, j.positionFrom.z) < 150.0)then
				...
				if(Vdist(pos.x, pos.y, pos.z, j.positionFrom.x, j.positionFrom.y, j.positionFrom.z) < 5.0)then
					...
					if(Vdist(pos.x, pos.y, pos.z, j.positionFrom.x, j.positionFrom.y, j.positionFrom.z) < 2.0)then
						ClearPrints()
						SetTextEntry_2("STRING")
						AddTextComponentString(" ~r~E~w~를 눌러 ".. j.positionFrom.nom " 이동하기 ")
			...
			if(Vdist(pos.x, pos.y, pos.z, j.positionTo.x, j.positionTo.y, j.positionTo.z) < 150.0)then
				...
				if(Vdist(pos.x, pos.y, pos.z, j.positionTo.x, j.positionTo.y, j.positionTo.z) < 5.0)then
					...
					if(Vdist(pos.x, pos.y, pos.z, j.positionTo.x, j.positionTo.y, j.positionTo.z) < 2.0)then
						ClearPrints()
						SetTextEntry_2("STRING")
						AddTextComponentString(" ~r~E~w~를 눌러 ".. j.positionTo.nom " 이동하기 ")
						DrawSubtitleTimed(2000, 1)
						if IsControlJustPressed(1, 38) then
							DoScreenFadeOut(1000)
							Citizen.Wait(2000)
