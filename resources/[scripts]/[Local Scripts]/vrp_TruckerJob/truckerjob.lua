...
local Truck = {"HAULER", "MAN", "ACTROS"}
local Trailer = {"TANKER", "TRAILERS", "TRAILERS2", "TRAILERLOGS"}

local MissionData = {
    [0] = {1212.4463, 2667.4351, 38.79, 500000}, --x,y,z,money
    [1] = {-410.6452331543,1177.8696289063,325.64175415039, 700000}, --x,y,z,money
    [2] = {2574.5144, 328.5554, 108.45, 1000000},
    [3] = {-2565.0894, 2345.8904, 33.06, 1200000},
    [4] = {1706.7966, 4943.9897, 42.16, 1400000},
    [5] = {196.5617, 6631.0967, 31.53, 1700000},
    [6] = {-567.66717529297,5356.8525390625,70.21849822998, 1900000},
    [7] = {-1038.5166015625,4921.43359375,206.30116271973, 2100000}
}
local MISSION = {}
MISSION.start = false
    ...
    if( GetDistanceBetweenCoords( playerCoords, TruckingCompany[0]["x"], TruckingCompany[0]["y"], TruckingCompany[0]["z"] ) < 10) then
            if(GUI.showStartText == false) then
                ...
                if(IsControlPressed(1, Keys["N+"]) and GUI.showMenu == false) then
                    --clear()
                    TriggerServerEvent('truckerJob:test')
                    GUI.menu = 0
                end
...
function GUI.init()
    ...
    GUI.addButton(0, "RON 유조 탱크", GUI.optionMisson, 0.35, 0.25, 0.3, 0.05 )
    GUI.addButton(0, "컨테이너", GUI.optionMisson, 0.35, 0.30, 0.3, 0.05 )
    ...
    GUI.addButton(1, "미션 1 [ 500,000$ ]", GUI.mission, 0.35, 0.25, 0.3, 0.05)
    GUI.addButton(1, "미션 3 [ 700,000$ ]", GUI.mission, 0.35, 0.30, 0.3, 0.05)
    GUI.addButton(1, "미션 4 [ 100,000$ ]", GUI.mission, 0.35, 0.35, 0.3, 0.05)
    GUI.addButton(1, "미션 5 [ 1,200,000$ ]", GUI.mission, 0.35, 0.40, 0.3, 0.05)
    GUI.addButton(1, "미션 6 [ 1,400,000$ ]", GUI.mission, 0.35, 0.45, 0.3, 0.05)
    GUI.addButton(1, "미션 7 [ 1,700,000$ ]", GUI.mission, 0.35, 0.50, 0.3, 0.05)
    GUI.addButton(1, "미션 8 [ 1,900,000$ ]", GUI.mission, 0.35, 0.55, 0.3, 0.05)
    GUI.addButton(1, "미션 9 [ 2,100,000$ ]", GUI.mission, 0.35, 0.60, 0.3, 0.05)
