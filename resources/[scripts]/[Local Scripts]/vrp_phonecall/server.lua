...
local function smsPlayer(player,choice)
	...
	if(money >= smsFee)then
		if(user_id == callingID)then
			vRPclient.notify(player, {"~r~자기 자신에게 전송할 수 없습니다!"})
		else
...
vRP.registerMenuBuilder({"phone", function(add)
	...
	--if user_id ~= nil then
		local choices = {}
		choices["*문자 전송"] = {function(player,choice)
			users = vRP.getUsers({})
			vRP.buildMenu({"Agenda", {player = player}, function(menu)
				...
				for k, v in pairs(users) do
					...
					if(playerName ~= myName)then
						...
						menu[playerName] = {smsPlayer, "문자 전송"}
					end
				end
				vRP.openMenu({player,menu})
			end})
		end,"접속중인 유저에게 문자를 전송합니다"}
		choices["*전화 걸기"] = {function(player,choice)
			users = vRP.getUsers({})
			vRP.buildMenu({"Agenda", {player = player}, function(menu)
				menu.name = "Agenda"
				menu.css={top="75px",header_color="rgba(200,0,0,0.75)"}
				...
				if(inPhoneCall[user_id] == nil)then
					...
					for k, v in pairs(users) do
						...
						if(playerName ~= myName)then
							playersToCall[playerName] = v
							menu[playerName] = {callPlayer, "플레이어에게 전화"}
						end
					end
				else
					menu["전화 끊기"] = {function(player,choice)
						local user_id = vRP.getUserId({player})
						vRPpc.cancelCall(user_id)
						vRP.closeMenu({player})
